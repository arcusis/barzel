        ...
        match self.proc.run(&mypy_cmd, &[".", "--ignore-missing-imports", "--no-error-summary"], root) {
            Ok(out) => {
                ...
                Ok(LayerResult {
                    ...
                    status,
                    findings,
                    metrics: LayerMetrics { failed: error_count, ..Default::default() },
            ...
            Err(e) => Ok(LayerResult {
                ...
                findings: vec![Finding {
                    ...
                    suggestion: Some("Install: `pip install mypy` or `uv add --dev mypy`".to_string()),
                    ..Default::default()
                }],
...
mod tests {
    ...
    #[test]
    fn name_is_mypy() { assert_eq!(MypyRunner::default().name(), "mypy"); }

    #[test]
    fn layer_is_logic() { assert!(matches!(MypyRunner::default().layer(), Layer::Logic)); }
    ...
    fn run_pass_returns_pass() {
        let r = MypyRunner { proc: Arc::new(MockProcessRunner::passing("Success: no issues found in 5 source files")) };
    ...
    fn run_fail_parses_type_errors() {
        ...
        let r = MypyRunner { proc: Arc::new(MockProcessRunner::failing(output)) };
        let result = r.run(&py_info()).unwrap();
        assert!(matches!(result.status, LayerStatus::Fail));
        assert!(!result.findings.is_empty());
        assert_eq!(result.findings[0].severity, Severity::High);
        assert!(result.findings[0].location.as_ref().unwrap().contains("app/main.py:42"));
    ...
    fn run_truncates_to_10_errors() {
        let lines: String = (1..=15).map(|i| {
            format!("src/f.py:{i}: error: Type error {i} [assignment]\n")
        }).collect();
        let r = MypyRunner { proc: Arc::new(MockProcessRunner::failing(&lines)) };
        let result = r.run(&py_info()).unwrap();
        let type_errors: Vec<_> = result.findings.iter().filter(|f| f.code.starts_with("MYPY_")).collect();
